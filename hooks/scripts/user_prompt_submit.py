...
# Resolve plugin root from CLAUDE_PLUGIN_ROOT or fall back to script-relative path
    ...
    if name_agent and "agent_name" not in session_data:
        ...
        try:
            result = subprocess.run(
                ["uv", "run", str(PLUGIN_ROOT / "hooks" / "scripts" / "utils" / "llm" / "anth.py"), "--agent-name"],
                capture_output=True,
                text=True,
                timeout=10,
            )
            ...
            try:
                result = subprocess.run(
                    ["uv", "run", str(PLUGIN_ROOT / "hooks" / "scripts" / "utils" / "llm" / "ollama.py"), "--agent-name"],
                    capture_output=True,
                    text=True,
